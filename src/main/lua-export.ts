...
export function buildLua(rows: TooltipRow[]): string {
  ...
  // Group: charName → itemId → { specs, ilvl, name, icon, updated, source, sourceType }
  type ItemInfo = {
  ...
  for (const row of rows) {
    ...
    // Keep freshest date and highest ilvl; fill in source/icon if not yet set
  ...
  for (const [charName, items] of Object.entries(byChar).sort()) {
    ...
    for (const [itemIdStr, info] of Object.entries(items).sort(([a], [b]) => Number(a) - Number(b))) {
      ...
      if (info.icon) lines.push(`      icon="${info.icon.replace(/"/g, '\\"')}",`)
      if (info.source) {
